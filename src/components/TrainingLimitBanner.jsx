...
function TrainingLimitBanner({ onUpgrade }) {
  const { planType, remainingTrainingRounds, trainingRoundsUsed } = useSubscription();
  ...
  const handleUpgrade = () => {
    if (onUpgrade) {
      ...
      window.open('https://aceedge.pro/pricing', '_blank');
    }
  };
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="flex-1">
          <p className="text-gray-200">
            <span className="font-bold text-yellow-300">{remainingTrainingRounds}</span> training round{remainingTrainingRounds !== 1 ? 's' : ''} remaining today
          </p>
        </div>
        <button
          onClick={handleUpgrade}
